...
const examples = [
  { label: "Appointment", text: "Hi, this is Alex. I’m calling to reschedule my appointment with the doctor from Monday to Thursday." },
  ...
  { label: "Open conversation", text: "Hello, could I speak with the doctor?" },
];
...
export function DeskApp() {
  ...
  const doctorName = provider?.doctor?.name ?? "The doctor";
  const doctorShort = doctorName.replace(/^(Dr\.?\s+)(\S+\s+)?(\S+)$/, "Dr. $3");
  const doctorSpecialty = provider?.doctor?.specialty ?? "";
  const doctorInitials = doctorName.replace(/^Dr\.?\s+/, "").split(/\s+/).map((part) => part[0] ?? "").join("").slice(0, 2).toUpperCase() || "DR";
  ...
  useEffect(() => {
    ...
    let saved: string | null = new URLSearchParams(location.search).get("session");
    if (!saved) { try { saved = sessionStorage.getItem("callsign.desk.session"); } catch { /* Storage is optional. */ } }
  ...
  return (
    <div className="desk-app">
      <header className="desk-header">
        ...
        <div className="desk-header-right"><button className="desk-icon-button" onClick={voice.toggle} aria-label={voice.enabled ? "Mute voice" : "Enable voice"} title={voice.enabled ? "Mute voice" : "Enable voice"}><Icon name={voice.enabled ? "volume" : "muted"} /></button><button className="desk-integrations" onClick={() => { setDialog("integrations"); void refreshStatus(); }}><Icon name="settings" size={16} /> Integrations</button><span className="desk-avatar" title={doctorName}>{doctorInitials}</span></div>
      </header>

      <main className="desk-main">
        <div className="desk-heading-row"><div><div className="desk-eyebrow"><span className="desk-live-dot" /> YOUR PRACTICE, PROTECTED</div><h1>A little distance from unwanted calls.</h1><p>Verify the agent. Apply your policy. Screen every message.</p></div><div className="desk-doctor-label"><span className="desk-avatar large">{doctorInitials}</span><div><strong>{doctorName}</strong><span>{doctorSpecialty || "Physician"} <span aria-hidden="true">·</span> Personal agent</span></div></div></div>
        ...
        {page === "workspace" ? <>
          ...
          <div className="desk-grid">
            <section className="desk-panel desk-caller" aria-labelledby="caller-heading">
              ...
              <div className="desk-caller-identity"><label htmlFor="caller-name">CALLER NAME <span>Self-reported</span></label><div><span className="desk-person-avatar">{(session?.callerName ?? callerName).trim().slice(0, 1).toUpperCase() || "?"}</span><input id="caller-name" value={session?.callerName ?? callerName} onChange={(event) => setCallerName(event.target.value)} maxLength={80} disabled={Boolean(session) || busy} autoComplete="off" /><Icon name="arrow" size={16} /><span className="desk-destination">{doctorShort}</span></div></div>
              <div className="desk-caller-feed" ref={callerFeed} role="log" aria-label="Caller conversation" aria-live="polite">
                {!session && !busy ? <div className="desk-empty-caller"><div className="desk-chat-illustration"><span><Icon name="message" size={26} /></span><span><Icon name="shield" size={20} /></span><i /><i /><i /></div><h3>Every conversation starts here.</h3><p>Say why you’re calling. Your words go to {doctorShort}’s agent first.</p><div className="desk-example-label">NEED A STARTING POINT?</div><div className="desk-examples">{examples.map((example) => <button key={example.label} onClick={() => setDraft(example.text)}>{example.label}<Icon name="arrow" size={12} /></button>)}</div></div> : <>
                  {visible?.transcript.map((line) => <div key={line.id} className={`desk-chat-line ${line.role}`}><div className="desk-chat-line-meta">{line.role === "caller" ? "You" : line.role === "doctor" ? `${doctorShort} · demo receiver` : "Callsign agent"}<span>{time(line.ts)}</span></div><div className="desk-chat-bubble">{line.text}</div></div>)}
              ...
              <form className="desk-compose" onSubmit={submit}>
                <label className="desk-sr-only" htmlFor="caller-message">Message to {doctorShort}’s agent</label><textarea id="caller-message" placeholder={terminal ? "Start a new session to try another conversation." : status === "ringing" ? `Waiting for ${doctorShort} to answer…` : connected ? "Continue your conversation…" : "Hi, I’m calling about…"} value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); if (draft.trim()) submit(event); } }} maxLength={2000} disabled={!canSend} rows={3} />
