 ...
 */
export function evaluatePolicy(input: { specialty?: string; doctorSpecialty: string; hasReceipt: boolean }): {
  ok: boolean;
  detail: string;
  ...
  if (!p.acceptCalls) return { ok: false, detail: `${p.note ?? "Dr. Patel is not taking calls"} · held in inbox`, evidence };
