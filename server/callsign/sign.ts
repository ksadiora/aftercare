...
import type { ReachRequest } from "./types.js";
...
/** Keys live under the configured directory (data/callsign-keys by default; data/ is gitignored). */
export const keysDir = () => config.keysDir;
...
export const keyPathFor = (agentName: string) => path.join(keysDir(), `${safeName(agentName)}.key`);
export const certPathFor = (agentName: string) => path.join(keysDir(), `${safeName(agentName)}.pem`);
